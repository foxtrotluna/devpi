...
def test_docs_raw_projectname(mapp, testapp):
    ...
    navlinks = dict(
        (l.text, l.attrs["href"]) for l in r.html.select(".subnavigation a")
    )
...
def test_docs_latest(mapp, testapp):
    ...
    assert r.text == '<!DOCTYPE html><html lang="en"><head><title>Docs</title></head><body>2.6</body></html>'
    ...
    r = testapp.xget(200, iframe.attrs['src'])
    assert r.text == '<!DOCTYPE html><html lang="en"><head><title>Docs</title></head><body>2.7</body></html>'
