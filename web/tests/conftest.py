...
def pytest_addoption(parser):
    parser.addoption("--fast", help="skip functional/slow tests", default=False,
                     action="store_true")
    parser.addoption("--html-validation", help="run HTML validation using vnu", default=False,
                     action="store_true")
...
def maketestapp(maketestapp, request):
    ...
    def do_request(self, *args, **kw):
        ...
        if resp.content_type in {"application/json", "text/css", "text/plain", "text/xml"}:
            return resp
        ...
        if (ct := resp.content_type) in {"text/html"}:
            ...
            with tempfile.NamedTemporaryFile(suffix=f".{ct.split('/')[-1]}") as f:
                ...
                try:
                    ...
                    print("\n".join(f"{i}:{l}" for i, l in enumerate(resp.text.splitlines(), start=1)))  # noqa: T201 - testing
                    raise
            return resp
