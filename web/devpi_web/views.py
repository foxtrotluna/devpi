...
def index_get(context, request):
    ...
    whitelist = []
    index_title = stage.ixconfig.get('title', None)
    title = f"{stage.name} ({index_title}) index" if index_title else f"{stage.name} index"
    result = dict(
        _context=context,
        title=title,
        nav_links=[dict(
    ...
    for project in sorted(stage.list_projects_perstage()):
        ...
        def add_version(versions, show_toxresults, project, version, is_pre_release):
            ...
            try:
                (name, version) = (
                    normalize_name(verdata["name"]),
                    verdata["version"])
            except KeyError:
                log.error("metadata for project %r %r empty: %s, skipping",
                          project, version, verdata)
                return
        ...
        if stable_version is not None:
            add_version(versions, show_toxresults, project, stable_version, is_pre_release=False)
        if latest_version is not None and stable_version != latest_version:
            add_version(versions, show_toxresults, project, latest_version, is_pre_release=True)
